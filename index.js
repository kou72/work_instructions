...
function handleFileClick() {
    Dialog.showOpenDialog(null, {
        ...
        defaultPath: '.',
        filters: [
            { name: 'XCELEファイル', extensions: ['xlsx', 'xlsm'] }
        ]
    }, (filePath) => {
...
function createJsonFile(filePath) {
    try {
        ...
        const category1 = Sheet1_json.filter((item) => item['カテゴリ階層'] === 1)
            .map((item) => {
                return pushObject(item);
            });
        const category2 = Sheet1_json.filter((item) => item['カテゴリ階層'] === 2)
            .map((item) => {
                return pushObject(item);
            });
        const category3 = Sheet1_json.filter((item) => item['カテゴリ階層'] === 3)
            .map((item) => {
                return pushObject(item);
            });
        const category4 = Sheet1_json.filter((item) => item['カテゴリ階層'] === 4)
            .map((item) => {
                return pushObject(item);
            })
        const category5 = Sheet1_json.filter((item) => item['カテゴリ階層'] === 5)
            .map((item) => {
                return pushObject(item);
            })
